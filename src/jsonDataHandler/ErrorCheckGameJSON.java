...
//		    System.err.println(correspondingExit);
		    try {
			  ...
			  //  Block of code to handle errors
				System.err.println("WARNING: The path between " + direction + " " + currentRoomName + " and " + oppositeDirections.get(direction) + " " + 
	...
	private void checkTakeableObjectWeight(JsonArray takeableObjects, String currentRoomName) {
		for(Object ele : takeableObjects) {
			...
			if(!obj.containsKey("weight")) {
				System.err.println("WARNING: " + currentRoomName + " takeableObjects " + name + " does not have weight");
			} else {
				if(obj.get("weight").toString() != obj.get("weight")) {
					System.err.println("WARNING: " + currentRoomName + " takeableObjects " + name + " weight isn't string. Put value in quotation marks.");
				}
			}
